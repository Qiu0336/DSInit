...
void CameraModel::distortpoint(const Eigen::Vector2d& p, Eigen::Vector2d& dp) const
{
    ...
    else if(distortion_model == Equidistant)
    {
        ...
        const double dtheta = theta*(1.0 + k1*theta2
                                  ...
                                  + k3*theta4*theta2
                                  + k4*theta4*theta4);
        const double scaling = (r > 1e-8) ? dtheta / r : 1.0;
        dp = scaling*p;
    }
    else if(distortion_model == None)
    {
...
void CameraModel::backprojectSymmetric(const Eigen::Vector2d& p_u, double& theta, double& phi) const
{
    double tol = 1e-10;
...
Eigen::Vector2d CameraModel::pixel2norm(const Eigen::Vector2d &pixel, bool ignore_dist)
{
    ...
    else
    {
        ...
        else if(distortion_model == Equidistant)
        {
            ...
            double theta, phi;
            backprojectSymmetric(p_d, theta, phi);
